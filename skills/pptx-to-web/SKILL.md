...
| `.cd` (안에 `h3`/`p`) | 글래스 카드. `style="--a:#..;--b:#.."`로 색상 |
| `.ic` | 카드 상단 아이콘 칩(이모지) |
| `.flow` + `.cd` | 가로 단계 플로우(카드 사이 → 화살표 자동) |
| `.stat` | 큰 강조 숫자(예: 60%) |
| inline `<svg>` | 다이어그램(허브-스포크/도넛/막대/팬인/분기). 텍스트만 많지 않게 시각화 |
| `.pill` / `.tag` | 키워드 알약 / NEW 배지 |
- **다이어그램 우선**: 구조·흐름·비교 장표는 텍스트 나열 대신 inline SVG로 그린다(원형 허브,
  도넛 차트, 막대, fan-in, 단계 분기). 그라데이션 `id='h'`(시안→퍼플) 재사용.
- 6색 테마는 슬라이드 순서대로 자동 로테이션. 카드별 `--a/--b`로 개별 지정 가능.
- 끝에 최신 Azure/Foundry 업데이트 슬라이드 자동 추가. 항목 많으면 카드 내부 스크롤로 1화면 유지.
