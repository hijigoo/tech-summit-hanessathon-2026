...
## 권장 워크플로우: 스크린샷 → AI 재해석 (content.json)
가장 완성도 높은 방식. PPTX 텍스트 추출에 의존하지 않고, **모든 슬라이드를
스크린샷(PNG)으로 렌더 → 에이전트가 직접 보고 → 슬라이드별 콘텐츠를 손으로
재해석해 `content.json`에 작성 → 빌더가 풀 디자인 카드로 렌더**한다. 빈 페이지 0 보장.
```bash
# 1) 전체 장표를 PNG로 캡처(soffice→PDF→PyMuPDF)
soffice --headless --convert-to pdf <input.pptx> --outdir /tmp
python3 -c "import fitz;d=fitz.open('/tmp/x.pdf');[p.get_pixmap(dpi=110).save(f'/tmp/shots/s{i+1:02d}.png') for i,p in enumerate(d)]"
# 2) 각 PNG를 view로 직접 보고 슬라이드별 title/html/note를 content.json에 작성
# 3) 빌드
python3 scripts/pptx2web_native.py --content content.json --updates docs/updates.json --out docs
```
- content.json 형식: `{title, slides:[{title, html, note} | {title, divider:true, note}]}`.
  `html`은 재해석한 본문(.gr/.cd/.flow/.pill/.stat/.ic 클래스 사용), 디바이더는 섹션 표지.
- 스크린샷은 임베드하지 않고 **이해용**일 뿐, HTML은 깔끔히 새로 만든다(1:1 복제 X).
- 끝에 최신 Azure/Foundry 업데이트 슬라이드 자동 추가. 빌드 후 headless 검증 권장.
...
- **랜드스케이프**: 각 슬라이드는 화면 1장(100vh)에 맞춘 가로형 카드, snap mandatory.
  항목이 많으면 자동 2~3단 컬럼, 길면 카드 내부 스크롤로 처리해 세로로 늘어지지 않게 한다.
